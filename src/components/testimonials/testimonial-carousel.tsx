  ...
  return (
    ...
    >
      <div className="relative">
        <div
          aria-hidden
          className="pointer-events-none absolute inset-y-0 left-0 z-10 w-8 bg-gradient-to-r from-background-primary to-transparent sm:w-12 lg:w-16"
        />
        <div
          aria-hidden
          className="pointer-events-none absolute inset-y-0 right-0 z-10 w-8 bg-gradient-to-l from-background-primary to-transparent sm:w-12 lg:w-16"
        />
        ...
        >
          {items.map((item, slideIndex) => (
            <div
              ...
              className="flex w-[min(20.5rem,calc(100vw-3rem))] shrink-0 snap-start sm:w-[min(24rem,calc(50vw-2.75rem))] lg:w-[min(26.5rem,calc(34vw-2rem))]"
            >
              <TestimonialCard item={item} active compact className="w-full" />
            </div>
          ))}
        </div>
      </div>
      ...
      {multiple ? (
        <div className="mx-auto mt-8 flex max-w-container items-center justify-center gap-4 px-gutter sm:justify-between sm:px-gutter-md lg:mt-10 lg:px-gutter-lg">
          <div className="flex items-center gap-2">
            {items.map((item, slideIndex) => (
              ...
              >
                {slideIndex === index ? (
                  <span
                    key={`${item.id}-${index}-${paused || reduceMotion || !inView || hidden ? "idle" : "run"}`}
                    className={cn(
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              aria-label={testimonialsContent.previousLabel}
            ...
            <button
              type="button"
              aria-label={testimonialsContent.nextLabel}
              onClick={() => goTo(index + 1)}
              className={cn(buttonVariants({ variant: "outline", size: "icon" }))}
            >
              <ChevronRight />
